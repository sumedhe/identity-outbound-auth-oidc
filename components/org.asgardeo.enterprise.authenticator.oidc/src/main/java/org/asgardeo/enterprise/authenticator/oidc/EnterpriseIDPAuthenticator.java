...
import org.asgardeo.enterprise.authenticator.oidc.model.OIDCStateInfo;
...
public class EnterpriseIDPAuthenticator extends AbstractApplicationAuthenticator
    ...
    public boolean canHandle(HttpServletRequest request) {
        ...
        if (EnterpriseIDPAuthenticatorConstants.LOGIN_TYPE.equals(getLoginType(request))) {
            return true;
        }
    ...
    private boolean hasErrorParamInRequest(HttpServletRequest request) {
        ...
        String error = request.getParameter(EnterpriseIDPAuthenticatorConstants.OAUTH2_ERROR);
        return StringUtils.isNotBlank(error);
    }

    private boolean hasCodeParamInRequest(HttpServletRequest request) {

        String code = request.getParameter(EnterpriseIDPAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE);
    ...
    protected String getLogoutUrl(Map<String, String> authenticatorProperties) {
        ...
        return authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.IdPConfParams.OIDC_LOGOUT_URL);
    }
    ...
    protected String getTokenEndpoint(Map<String, String> authenticatorProperties) {
        ...
        return authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.OAUTH2_TOKEN_URL);
    }
    ...
    protected String getScope(String scope, Map<String, String> authenticatorProperties) {
        ...
        if (StringUtils.isBlank(scope)) {
            scope = EnterpriseIDPAuthenticatorConstants.OAUTH_OIDC_SCOPE;
        }
        return scope;
    }
    ...
    protected String getAuthenticateUser(AuthenticationContext context, Map<String, Object> oidcClaims,
        ...
        return (String) oidcClaims.get(EnterpriseIDPAuthenticatorConstants.Claim.SUB);
    }
    ...
    protected Map<ClaimMapping, String> getSubjectAttributes(OAuthClientResponse token,
        ...
        try {
            String accessToken = token.getParam(EnterpriseIDPAuthenticatorConstants.ACCESS_TOKEN);
    ...
    public ArrayList<String> getOrganizations(String emailDomain) throws Exception {

        ArrayList<String> organizations = new ArrayList<>();
        try {
            Connection connection = IdentityDatabaseUtil.getDBConnection(false);
            PreparedStatement ps;
            ps = connection.prepareStatement("SELECT OUTBOUND_SP_TENANT_ID FROM ASG_ENTERPRISE_LOGIN_EMAIL_DOMAIN " +
                    "WHERE EMAIL_DOMAIN = ? ");
            ps.setString(1, emailDomain);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                int outboundSpTenantIdID = rs.getInt("OUTBOUND_SP_TENANT_ID");
                String organization = IdentityTenantUtil.getTenantDomain(outboundSpTenantIdID);
     ...
     */
    private InboundAuthenticationRequestConfig getAuthenticationConfig(ServiceProvider application) {
    ...
    private int getOutboundSpId(int outboundSpTenantId) throws Exception {

        ArrayList<Integer> spIds = new ArrayList<>();
        try {
            Connection connection = IdentityDatabaseUtil.getDBConnection(false);
            PreparedStatement ps;
            ps = connection.prepareStatement("SELECT OUTBOUND_SP_ID FROM ASG_ENTERPRISE_LOGIN_MGT " +
                    "WHERE OUTBOUND_SP_TENANT_ID = ? ");
            ps.setInt(1, outboundSpTenantId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                int outboundSpID = rs.getInt("OUTBOUND_SP_ID");
                spIds.add(outboundSpID);
            }
            return spIds.get(0);
        } catch (SQLException e) {
            throw new Exception(e);
    ...
    private Map<String, String> resolveTenantIDP(AuthenticationContext context,
                                                 Map<String, String> authenticatorProperties) {
        try {
            String username = null;

            Map<String, String> runtimeParams = getRuntimeParams(context);
            if (StringUtils.isNotBlank(runtimeParams.get("emailFromRequest"))) {
                username = runtimeParams.get("emailFromRequest");
            }
//            String username = context.getLastAuthenticatedUser().getUserName();
            String emailDomain = username.split("@")[1];
            ArrayList<String> org = getOrganizations(emailDomain);
            if (org.size() == 1) {
                int outboundSpTenantID = IdentityTenantUtil.getTenantId(org.get(0));

                    int outboundSpId = getOutboundSpId(outboundSpTenantID);
                    ApplicationManagementService applicationManagementService =
                            EnterpriseIDPAuthenticatorDataHolder.getInstance().getApplicationManagementService();
                    ...
                    if (openIDConnectConfiguration != null) {
                        ...
                        authenticatorProperties.put(EnterpriseIDPAuthenticatorConstants.CLIENT_ID, clientId);

                        OAuthAdminServiceImpl oAuthAdminServiceImpl =
                                EnterpriseIDPAuthenticatorDataHolder.getInstance().getOAuthAdminService();
                        ...
                        authenticatorProperties.put(EnterpriseIDPAuthenticatorConstants.OAUTH2_AUTHZ_URL,
                                authzEndpoint.replace("{tenant}", org.get(0)));

                        String tokenEndpoint = authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.OAUTH2_TOKEN_URL);
                        authenticatorProperties.put(EnterpriseIDPAuthenticatorConstants.OAUTH2_TOKEN_URL,
                                tokenEndpoint.replace("{tenant}", org.get(0)));

                        authenticatorProperties.put(IdentityApplicationConstants.OAuth2.CALLBACK_URL,
                                oauthApp.getCallbackUrl());
                        authenticatorProperties.put(EnterpriseIDPAuthenticatorConstants.CLIENT_SECRET,
                                oauthApp.getOauthConsumerSecret());

                    }
                }
            } catch(Exception e){
                log.error("Error while resolving");
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request, HttpServletResponse response,
            ...
            try {
                ...
                if (authenticatorProperties != null) {
                    String clientId = authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.CLIENT_ID);
                ...
                if (StringUtils.isNotBlank(queryString)) {
                    ...
                    context.setProperty(EnterpriseIDPAuthenticatorConstants.OIDC_QUERY_PARAM_MAP_PROPERTY_KEY, paramValueMap);
                }
                ...
                if (StringUtils.isNotBlank(queryString) && queryString.toLowerCase().contains("scope=") && queryString
                    ...
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setResponseType(EnterpriseIDPAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE).setState(state)
                            .buildQueryMessage();
                    ...
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setResponseType(EnterpriseIDPAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE)
                            .setScope(EnterpriseIDPAuthenticatorConstants.OAUTH_OIDC_SCOPE).setState(state).buildQueryMessage();

                } else {
                    authzRequest = OAuthClientRequest.authorizationLocation(authorizationEP).setClientId(clientId)
                            .setRedirectURI(callbackurl)
                            .setResponseType(EnterpriseIDPAuthenticatorConstants.OAUTH2_GRANT_TYPE_CODE).setScope(scope)
            ...
            throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.BUILDING_AUTHORIZATION_CODE_REQUEST_FAILED.getCode(),
                    e.getMessage(), e);
        } catch (IOException e) {
    ...
    private String getStateParameter(AuthenticationContext context, Map<String, String> authenticatorProperties) {
        ...
        String state = context.getContextIdentifier() + "," + EnterpriseIDPAuthenticatorConstants.LOGIN_TYPE;
    ...
    private String getOIDCAuthzEndpoint(Map<String, String> authenticatorProperties) {
        ...
        if (StringUtils.isBlank(authorizationEP)) {
            authorizationEP = authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.OAUTH2_AUTHZ_URL);
        }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            String accessToken = oAuthResponse.getParam(EnterpriseIDPAuthenticatorConstants.ACCESS_TOKEN);

            if (StringUtils.isBlank(accessToken)) {
                throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.ACCESS_TOKEN_EMPTY_OR_NULL.getCode(),
                        OIDCErrorConstants.ErrorMessages.ACCESS_TOKEN_EMPTY_OR_NULL.getMessage());
            }

            String idToken = oAuthResponse.getParam(EnterpriseIDPAuthenticatorConstants.ID_TOKEN);
//            Map<String, String> authenticatorProperties = context.getAuthenticatorProperties();
            if (StringUtils.isBlank(idToken) && requiredIDToken(authenticatorProperties)) {
                throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.ID_TOKEN_MISSED_IN_OIDC_RESPONSE.getCode(),
                        String.format(OIDCErrorConstants.ErrorMessages.ID_TOKEN_MISSED_IN_OIDC_RESPONSE.getMessage(),
                                ...
                                authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.CLIENT_ID)));
            }
            ...
            context.setProperty(EnterpriseIDPAuthenticatorConstants.ACCESS_TOKEN, accessToken);
            ...
            if (StringUtils.isNotBlank(idToken)) {
                ...
                if (jsonObject == null) {
                    ...
                    throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.DECODED_JSON_OBJECT_IS_NULL.getCode(),
                            errorMessage);
                }

                String idpName = context.getExternalIdP().getIdPName();
                String sidClaim = (String) jsonObject.get(EnterpriseIDPAuthenticatorConstants.Claim.SID);
                ...
                throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.AUTHENTICATION_PROCESS_FAILED.getCode(),
                        OIDCErrorConstants.ErrorMessages.AUTHENTICATION_PROCESS_FAILED.getMessage(), context.getSubject(), e);
            }
        }
        ...
        protected void initiateLogoutRequest(HttpServletRequest request, HttpServletResponse response,
            ...
            if (isLogoutEnabled(context)) {
                ...
                if (StringUtils.isNotBlank(idTokenHint)) {
                    paramMap.put(EnterpriseIDPAuthenticatorConstants.ID_TOKEN_HINT, idTokenHint);
                }

                String callback = getCallbackUrl(context.getAuthenticatorProperties());
                paramMap.put(EnterpriseIDPAuthenticatorConstants.POST_LOGOUT_REDIRECT_URI, callback);

                String sessionID = getStateParameter(context, context.getAuthenticatorProperties());
                paramMap.put(EnterpriseIDPAuthenticatorConstants.OAUTH2_PARAM_STATE, sessionID);
    ...
    private Map<String, Object> getIdTokenClaims(AuthenticationContext context, String idToken) {
        ...
        context.setProperty(EnterpriseIDPAuthenticatorConstants.ID_TOKEN, idToken);
    ...
    private String getMultiAttributeSeparator(AuthenticationContext context, String authenticatedUserId)
        ...
        try {
            ...
            int tenantId = EnterpriseIDPAuthenticatorDataHolder.getInstance().getRealmService().getTenantManager()
                    .getTenantId(tenantDomain);
            UserRealm userRealm = EnterpriseIDPAuthenticatorDataHolder.getInstance().getRealmService()
            ...
            throw new AuthenticationFailedException(OIDCErrorConstants.ErrorMessages.RETRIEVING_MULTI_ATTRIBUTE_SEPARATOR_FAILED.getCode(),
    ...
    protected void buildClaimMappings(Map<ClaimMapping, String> claims, Map.Entry<String, Object> entry,
        ...
        authzResponse) throws AuthenticationFailedException {
            ...
            String clientId = authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.CLIENT_ID);
            String clientSecret = authenticatorProperties.get(EnterpriseIDPAuthenticatorConstants.CLIENT_SECRET);
            ...
            boolean isHTTPBasicAuth = Boolean.parseBoolean(authenticatorProperties.get(
                    EnterpriseIDPAuthenticatorConstants
            ...
            try {
                ...
                if (accessTokenRequest != null) {
                    ...
                    accessTokenRequest.addHeader(EnterpriseIDPAuthenticatorConstants.HTTP_ORIGIN_HEADER, serverURL);
                }
            } catch (OAuthSystemException e) {
                if (log.isDebugEnabled()) {
                    log.debug(String.format(OIDCErrorConstants.ErrorMessages.BUILDING_ACCESS_TOKEN_REQUEST_FAILED.getMessage(),
                            tokenEndPoint), e);
                }
        ...
        protected OAuthClientResponse getOauthResponse(OAuthClient oAuthClient, OAuthClientRequest accessRequest)
            ...
            try {
                ...
                throw new AuthenticationFailedException(
                        OIDCErrorConstants.ErrorMessages.REQUESTING_ACCESS_TOKEN_FAILED.getCode(), e.getMessage(), e);
            }
        ...
        public String getContextIdentifier(HttpServletRequest request) {
            ...
            String state = request.getParameter(EnterpriseIDPAuthenticatorConstants.OAUTH2_PARAM_STATE);
            if (state != null) {
        ...
        try {
            ...
            throw new AuthenticationFailedException(
                    OIDCErrorConstants.ErrorMessages.EXECUTING_CLAIM_TRANSFORMATION_FOR_IDP_FAILED.getCode(),
                    String.format(OIDCErrorConstants.ErrorMessages.EXECUTING_CLAIM_TRANSFORMATION_FOR_IDP_FAILED.getMessage(),
    ...
    private String getUserIdClaimUriInOIDCDialect(String userIdClaimInLocalDialect, String spTenantDomain)
        ...
        List<ExternalClaim> externalClaims = EnterpriseIDPAuthenticatorDataHolder.getInstance()
    ...
    private String interpretQueryString(AuthenticationContext context, String queryString,
        ...
        if (queryString.contains(EnterpriseIDPAuthenticatorConstants.AUTH_PARAM)) {
    ...
    private String getQueryStringWithAuthenticatorParam(AuthenticationContext context, String queryString) {
        ...
        Matcher matcher = Pattern.compile(EnterpriseIDPAuthenticatorConstants.DYNAMIC_AUTH_PARAMS_LOOKUP_REGEX)
                .matcher(queryString);
        String value = "";
        while (matcher.find()) {
        ...
        @SuppressWarnings({"unchecked"}) Map<String, String> paramValueMap = (Map<String, String>) context
                .getProperty(EnterpriseIDPAuthenticatorConstants.OIDC_QUERY_PARAM_MAP_PROPERTY_KEY);

        if (MapUtils.isNotEmpty(paramValueMap) && paramValueMap.containsKey(EnterpriseIDPAuthenticatorConstants.REDIRECT_URI)) {
            return paramValueMap.get(EnterpriseIDPAuthenticatorConstants.REDIRECT_URI);
        }
